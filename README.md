...
<head>
<meta charset="UTF-8">
<title>ગ્રામ અવાજ / Village Voice</title>
<style>
...
<nav>
    ...
    <button onclick="showSection('submit')">🕊️ New Complaints</button>
    <button onclick="showSection('view')">📢 Complaints</button>
    <button onclick="showSection('graph')">📊 Results</button>
...
<h2>Choose Language / ભાષા પસંદ કરો</h2>
<button class="language-btn lang-gu" onclick="setLanguage('gu')">ગુજરાતી</button>
...
<button class="main voice" onclick="startVoice()" id="voiceBtn">🎤 Speak Complaint</button>
<button class="main" onclick="addProblem()" id="sendBtn">📨 Send</button>
</div>
</section>
...
<div class="helpline">🚰 Vijaybhai Patel – <a href="tel:9876543210">9876543210</a></div>
<div class="helpline">⚡ Maheshbhai Solanki – <a href="tel:9898012345">9898012345</a></div>
<div class="helpline">🧹 Rekhaben Chaudhary – <a href="tel:9723456789">9723456789</a></div>
<div class="helpline">🚑 Dr. Kiranbhai Shah – <a href="tel:9811122334">9811122334</a></div>
<div class="helpline">🛣️ Nitinbhai Rathod – <a href="tel:9667788990">9667788990</a></div>
<div class="helpline">👮 Hardikbhai Parmar – <a href="tel:9900123456">9900123456</a></div>
</div>
</section>
</main>
...
function updateLanguageContent(){
 if(language==='gu'){
     document.getElementById('homeTitle').innerText="ગ્રામ અવાજમાં આપનું સ્વાગત છે";
     document.getElementById('homeText').innerText="કૃપા કરીને તમારી સમસ્યા વિસ્તારના નામ સાથે ફરિયાદ લખો";
     document.getElementById('homeBtn').innerText="ફરિયાદ કરવા જાઓ";
     document.getElementById('submitTitle').innerText="નવી ફરિયાદ";
     document.getElementById('voiceBtn').innerText="🎤 બોલીને ફરિયાદ કરો";
     document.getElementById('sendBtn').innerText="📨 મોકલો";
     document.getElementById('viewTitle').innerText="ગામની ફરિયાદો";
     document.getElementById('graphTitle').innerText="સહમતિ પરિણામ";
     document.getElementById('helpTitle').innerText="જરૂરી સહાય સંપર્ક";
     document.getElementById('currentLang').innerText = "વર્તમાન ભાષા: ગુજરાતી";
 } else {
     ...
     document.getElementById('voiceBtn').innerText="🎤 Speak Complaint";
...
function addProblem(){
 ...
 if(!t){ alert(language==='gu'?"ફરિયાદ લખો":"Write a complaint"); return; }
 problems.push({text:t,like:0,dislike:0});
 document.getElementById('text').value='';
 userVotes = {}; 
 renderProblems();
 speak(language==='gu'?"તમારી ફરિયાદ નોંધાઈ ગઈ છે":"Your complaint has been submitted");
}
...
function renderProblems(){
 ...
 problems.forEach((p,i)=>{
  l.innerHTML+=`<div class="problem">
   <p>${p.text}</p>
   <button class="vote-btn like" onclick="vote(${i},'like')" ${userVotes[i]==='like'?'disabled':''}>👍 ${p.like}</button>
   <button class="vote-btn dislike" onclick="vote(${i},'dislike')" ${userVotes[i]==='dislike'?'disabled':''}>👎 ${p.dislike}</button>
  </div>`;
 });
}
...
// --- Voice input ---
function startVoice(){
 if(!('webkitSpeechRecognition' in window)){ alert(language==='gu'?"Google Chrome વાપરો":"Use Google Chrome"); return; }
